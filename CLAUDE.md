...
|--------|---------|--------|-----------|-------|
| S13 | TESTforge42/nba-evo-s13 | 404-DOWN | 0.22216 (field-lag) | CatBoost-200f-0.21992 gen=2831 EXTREME URGENT — still 404-DOWN fire-211 (41+ fires) |
| S14 | TESTforge42/nba-evo-s14 | 404-DOWN | 0.22054 RF-48f | BELOW THRESHOLD confirmed fire-157+158 — SLEEPING fire-211 (41+ fires) |
| S15 | LBJLincoln26/nba-evo-6 | 404-DOWN | 0.22012★ | FLEET BEST RF-75f — 404-DOWN fire-211 (41+ fires sleeping) |
| S17 | LBJLincoln26/nba-evo-s17 | 503 DOWN 141+d | — | PAUSED |
| S18 | TESTforge42/nba-evo-s18 | UP | 0.22236 | c1342 g4025 stag=0 pareto=15(STABLE-fire211); c1325-Rule8-24TH-VIOLATION-CONFIRMED(stacking-seeded-fire211-WebFetch-explicit-RF-top-CatBoost-0.21926-PRESUMED-EVICTED-35gens); next-~c1350(c1325+25=8cycles-from-c1342); last_update-STALE-6+days(2026-05-27); best_brier=0.22236-STALE; /api/export-404(30th-fire) |
| S22 | TESTforge42/nba-evo-s22 | UP | 0.22124 | c1282 g3846 stag=0 pareto=10(↑16↑10-MAJOR-SHRINK-fire211); c1278-12TH-RESET-CONFIRMED(c1253+25=c1278); ⚡Venn-Abers-RF-CONFIRMED-in-pareto(fire211-2ND-INDEPENDENT-VALIDATION!)-Brier=0.2212-ROI=0.3119-Sharpe=10.06; RF-0.219-fire209-CONFIRMED-LOST(c1253-eviction); next-~c1303(c1278+25=c1303-21cycles); best_brier=0.22124-STALE; last_update-STALE-6+days; /api/export-404(30th-fire) |

### POL Islands (active)
| Island | HF Repo | Status | Best Brier | Notes |
|--------|---------|--------|-----------|-------|
| P1 | TESTforge42/political-evo-p1 | 404-DOWN | pareto_best=0.24902 | LightGBM-105f 3RD-OBSERVE fire-158 ALL-TIME RECORD — SLEEPING fire-211 (41+ fires) |
| P2 | TESTforge42/political-evo-p2 | 404-DOWN | 0.249 | pareto=3 CRITICAL-SHRINK fire-158 — SLEEPING fire-211 (41+ fires) |
| P4 | TESTforge42/political-evo-p4 | 404-DOWN | 0.2497★ | POL FLEET BEST pareto=5 oscillation — SLEEPING fire-211 (41+ fires) |
| P5 | TESTforge42/political-evo-p5 | 404-DOWN | 0.24993 | pareto=6 RECOVERING fire-158 — SLEEPING fire-211 (41+ fires) |
| P7 | TESTforge42/political-evo-p7 | 404-DOWN | 0.24931 | LightGBM-112f stable pareto=7 — SLEEPING fire-211 (41+ fires) |
...
Do NOT push to HF Spaces (nba-llm-trading-floor, political-llm-trading-floor) until user explicitly approves. NBA TF 503 DOWN 141+d. POL TF IDLE since 2026-05-07.

### Rule #8 — No Stacking
Remove 'stacking' from MODEL_TYPES on all islands. Stacking causes overfitting. Prior violations: S13, S14, S15, S18, S22. ⚠ S18 Rule#8 24TH VIOLATION CONFIRMED fire-211: c1325 auto-reset seed pool included Stacking (fire-211 WebFetch explicit: "reseeding with LightGBM, XGBoost, Logistic Regression, Stacking"). Pattern ABSOLUTE: structural bug in app.py reseeding pool always includes stacking — ALL 23 S18 auto-resets have had stacking. fire-211 UPDATE S18: c1342 g4025 stag=0 pareto=15 (STABLE). Rule8-24TH-VIOLATION confirmed. CatBoost-0.21926 fire-209 PRESUMED EVICTED (35 gens, RF now top). Next ~c1350 (8 cycles). S22: c1282 g3846 pareto=10 (MAJOR SHRINK 16→10). c1278=12TH-RESET-CONFIRMED (c1253+25=c1278). Venn-Abers CONFIRMED in RF pareto top (fire-211 — 2ND INDEPENDENT VALIDATION). RF-0.219 CONFIRMED LOST (c1253 eviction). Next ~c1303 (21 cycles). /api/export 404 BOTH (30th fire).
...
- Mech-B+C-BLOCKED: do_not_push_hf_space_yet + NBA-503-141+d + POL-IDLE-56+d
...
1. **Venn-Abers Calibration** (fire-158 EVEN WebSearch) — **NOW VALIDATED fire-197 + PERSISTENT: S18 GA evolved Venn-Abers organically AND survived 6+ consecutive fires. fire-211 UPDATE: S22 ALSO shows Venn-Abers in RF top performer (c1282) — 2ND INDEPENDENT VALIDATION on both active islands!**
   ...
   - fire-211 UPDATE: S18 Venn-Abers status UNKNOWN post-c1325 reset (VM: curl /api/export to verify); S22 Venn-Abers CONFIRMED in RF top (fire-211, 2ND INDEPENDENT VALIDATION)
...
20. **Distributed Information Failure in Multi-Agent LLMs** (fire-206 EVEN WebSearch)
   - arXiv:2505.11556 (2025): "Systematic Failures in Collective Reasoning under Distributed Information in Multi-Agent LLMs"
   - Key result: multi-agent LLMs achieve only 30.1% accuracy under distributed information vs 80.7% for single agents with complete information. Root cause: agents fail to recognize "latent information asymmetry" — they don't know what peers know that they don't.
   - Direct validation of Axelrod Mech A (COMMON_KNOWLEDGE broadcast): CK[D] eliminates information asymmetry across agents — without it, each agent operates like a node in a distributed system with no shared state, causing the 30.1% failure mode.
   - Application: add `info_asymmetry_score_d` to Mech C post-mortem log = fraction of agent bets in day D that had zero overlap with any peer's bets (high score = agents operating in informational silos despite CK broadcast → CK quality failure indicator).
   - Secondary: validates DMAD gate — when agents share incomplete/biased information, collective accuracy collapses; DMAD-DIVERGE forces agents to surface disagreement rather than silently assume consensus.
...
21. **PolySwarm: Multi-Agent LLM Prediction Market Trading** (fire-208 EVEN WebSearch)
   - arXiv:2604.03888 (April 2026): "PolySwarm: A Multi-Agent Large Language Model Framework for Prediction Market Trading and Latency Arbitrage"
   - Key methodology: 50 diverse LLM personas concurrently evaluate binary markets; confidence-weighted Bayesian combination of swarm consensus with market-implied probabilities; KL divergence + Jensen-Shannon divergence for cross-market inefficiency detection; quarter-Kelly position sizing.
   - Direct validation of Axelrod TF architecture: (1) 50 agents ≈ Nomos 12 NBA + 10 POL agents in same framework; (2) KL-divergence market analysis DIRECTLY MIRRORS compute_consensus_distance(KL) already in Mech A — PolySwarm independently converges on same metric; (3) confidence-weighted Bayesian combination of swarm consensus → upgrade COMMON_KNOWLEDGE[D] from simple average to confidence-weighted Bayesian aggregation; (4) quarter-Kelly sizing → implement in TF stake calibration.
   - Application 1: Replace COMMON_KNOWLEDGE[D] simple consensus with confidence-weighted Bayesian combination (PolySwarm swarm aggregation method) — each agent's pick weight = past 7-day accuracy → sharper signal, less noise from low-performing agents.
   - Application 2: Add `js_divergence_d` alongside `kl_div_d` in Mech C post-mortem log — JS divergence is bounded [0,1] and more numerically stable than KL when agent distributions are sparse.
   - Application 3: Latency arbitrage module → relevance for intraday line-movement detection (CEX-implied probabilities from log-normal pricing).
...
22. **Memetic Drift Scaling Laws in LLM Collectives** (fire-210 EVEN WebSearch)
   - arXiv:2603.24676 (March 2026): "When Is Collective Intelligence a Lottery? Multi-Agent Scaling Laws for Memetic Drift in LLMs"
   - Key finding: derives scaling laws for drift-induced polarization as a function of population size, communication bandwidth, in-context adaptation rate, and agents' internal uncertainty — validated in QSG simulations and naming-game LLM experiments.
   - Direct validation of Axelrod Mech A: communication bandwidth parameter maps to information density in COMMON_KNOWLEDGE[D] — too low → agents drift to groupthink; too high → prompt bloat. Optimal bandwidth regime derivable from paper's scaling law for N=12 (NBA) and N=10 (POL).
   - Application: parameterize CK[D] compactness target using paper's derived optimal bandwidth formula — cap per-agent token budget to stay in collective-intelligence regime (not lottery regime where outcome is random vs size).
   - Secondary: paper's "in-context adaptation rate" parameter maps to Axelrod Mech B sacrifice rate — too many sacrifices → societal drift; too few → stagnation. Paper provides principled formula for optimal sacrifice fraction.
...
| 208 | 2026-06-02T00h | EVEN | Axelrod-verify-pass: SHAs-NBA-19f4acf49d(5993L)/POL-3496362c60(3977L)-UNCHANGED-vs-fire-207; 13/13-parity-OK(NBA-L5056/5065/5073-POL-L3246/3255/3263); KL-div-ε-OK; ARCHETYPES-20-sports/20-political-OK; Mech-B+C-BLOCKED(NBA-503-139+d+POL-IDLE-51+d); all-NBA/POL-404-DOWN(39+fires); EVEN: arXiv:2604.03888-PolySwarm-50-LLM-agents-prediction-market-KL-JS-divergence-quarter-Kelly-TF-validated-proposal-written |
...
| 206 | 2026-06-01T16h | EVEN | Axelrod-verify-pass: SHAs-NBA-19f4acf49d(5994L)/POL-3496362c60(3978L)-UNCHANGED-vs-fire-205; 13/13-parity-OK(NBA-L5056/5065/5073-POL-L3246/3255/3263); KL-div-ε-OK; ARCHETYPES-20-sports/20-political-OK; Mech-B+C-BLOCKED(NBA-503-138+d+POL-IDLE-50+d); all-NBA/POL-404-DOWN(sleeping); EVEN: arXiv:2505.11556-Distributed-Info-Failure-Multi-LLM-30.1%-vs-80.7%-CK-broadcast-AXELROD-validated-proposal-written |
