...
| S18 | TESTforge42/nba-evo-s18 | EVOLVING | 0.22326 | **fire-275**: c=903 g=2707 stag=0 pareto=14. **CatBoost-0.22059×2 BELOW GATE 0.22085 — CHECKPOINT URGENT!** RULE8-VIOLATION: stacking-37f-0.24738 rank4. **38TH RESET ~c917 (14cy = ~3h)**. VM: (1) REMOVE stacking from app.py IMMEDIATELY, (2) DIRECT PICKLE SAVE CatBoost-0.22059. |
| S22 | TESTforge42/nba-evo-s22 | EVOLVING | 0.2207 | **fire-275**: c=314 g=942 stag=0 pareto=18. ⚡⚡ **RF-200f-~0.219 LIVE (~10bp below fleet best 0.22012!)**. brier_trend~0.231 — **RULE8-CLEAN: 11TH+12TH RESETS (c278+c303) BOTH CONFIRMED CLEAN**. **13TH RESET ~c328 (14cy = ~3h)**. VM: DIRECT PICKLE SAVE RF-~0.219 IMMEDIATELY. |

### POL Islands (active)
| Island | HF Repo | Status | Best Brier | Notes |
|--------|---------|--------|-----------|-------|
| P1 | TESTforge42/political-evo-p1 | 404-DOWN | pareto_best=0.24902 | RULE9 RESOLVED fire-256. SLEEPING 98+ fires. |
| P2 | TESTforge42/political-evo-p2 | 404-DOWN | 0.24903 | **fire-275**: 404-DOWN (98+ fires). history brier=0.24903 BELOW POL GATE 0.2497 — CHECKPOINT URGENT! RULE9 RESOLVED. VM: confirm checkpoint saved. |
| P4 | lbjlincoln/political-alpha-4 | EVOLVING | 0.2497★ | **fire-275**: c=3223 g=9667 stag=0. **pareto=10 RECOVERY** (from critical pareto=2 fire-273!). **lgbm_brier NOW in model_types** (partial RULE9 compliance). Post-reset gen0-gen2 only. VM: add standard 'lightgbm' for FULL RULE9 compliance. |
| P5 | lbjlincoln/political-alpha-5 | EVOLVING (SAVING) | 0.25316 | **fire-275**: c=1612 pareto=4 **SHRINKING**. **RULE9 VIOLATION** (LGB ABSENT, catboost_specialist dominates). XGB_brier-0.24989 (1.9bp above POL gate 0.2497). VM: add LightGBM IMMEDIATELY. |
| P7 | lbjlincoln/political-alpha-7 | EVOLVING | 0.25597 | **fire-275**: c=97 g=290 stag=18. **DIVERSIFY SENT+QUEUED** (stag=18 exceeded threshold=15). **RULE9 VIOLATION** (NO LightGBM). XGB_brier-0.24987/gen255 (1.7bp above POL gate). VM: (1) add LightGBM IMMEDIATELY, (2) pickle XGB_brier-0.24987. |
...
52. **Set-Preserving Calibration from Conformal P-Values to E-Values** (fire-276 EVEN WebSearch)
    - arXiv:2606.03600 (Jun 2026): "Set-Preserving Calibration from Conformal P-Values to E-Values"
    - Key finding: Develops an e-value formulation that enables principled use of e-value merging and randomization in cross-conformal prediction and conformal aggregation; e-value-based calibration satisfies the desired 1-α coverage guarantee while improving efficiency over standard p-value baselines; "set-preserving" means the prediction sets from conformal p-values are mapped to e-values WITHOUT losing information from the original set structure
    - Why e-values matter for NBA/POL: e-values support anytime-valid testing (no need to fix sample size in advance), multiplicative merging across islands WITHOUT independence assumption (unlike Fisher's p-value combination), and sequential model promotion decisions — directly applicable to multi-island Pareto fusion where calibration data accumulates online
    - Application 1: Replace isotonic calibration in engine.py with e-value calibration wrapper (~40 lines in `calibration/e_value_calibrator.py`) — drop-in replacement that improves efficiency under distribution shift
    - Application 2: Cross-island evidence aggregation — multiply e-values from S18/S22/evo4/evo5 to get a joint e-value for fleet-best claim (replaces ad-hoc rank-averaging with principled anytime-valid test); add `joint_e_value` to /api/export
    - Application 3: Replace split-CP in compute_consensus_distance (Mech C) with e-value distance metric — anytime-valid KL-divergence bounds without splitting calibration data; enables running KL estimate per-fire rather than per-calibration-batch
    - Application 4: E-value stopping criterion for evolution — island stops sending /api/checkpoint candidates when joint_e_value < threshold (replaces ad-hoc field-lag detection with principled early stopping)
    - Application 5: Port to political_engine.py — e-value merging for rare POL event calibration (election night label corrections = exactly the "corrupted feedback" case where anytime-valid testing is critical, complements arXiv:2605.20515 priority=118)
    - Library: scipy.stats (already available — no new dependencies; e-value computation is ~10 lines)
    - Expected improvement: 0.001-0.002 Brier + anytime-valid island promotion decisions + more principled multi-island fusion
    - Proposal: data/research-proposals/sota-set-preserving-calibration-e-values-fire276.md
    - Work-queue: vm-research-set-preserving-e-values-fire276 (priority=131)
...
### MODEL_TYPES Status
| Island | Current (fire-276) | Status |
|--------|---------|--------|
| P1 | extra_trees, xgboost_brier, lightgbm_brier, lightgbm | ✅ RULE9 RESOLVED fire-256 — 404-DOWN 99+ fires |
| P2 | lightgbm is best (xgboost, lightgbm, catboost, xgboost_brier) | ✅ RESOLVED fire-255 — 404-DOWN 99+ fires |
| P4 | lgbm_brier NOW in model_types! (c3223, post-reset) — standard lightgbm still missing | ⚠️ PARTIAL RULE9 carry-forward fire-276. pareto=10 RECOVERY. VM: add standard 'lightgbm' for full compliance |
| P5 | LGB ABSENT at c=1612 (catboost_specialist + xgboost_brier) | ⚠️ RULE9 VIOLATION carry-forward fire-276 (pareto=4 SHRINKING). VM: add LightGBM IMMEDIATELY |
| P7 | c=97, stag=18, DIVERSIFY SENT fire-275 — NO LightGBM | ⚠️ RULE9 VIOLATION carry-forward fire-276. DIVERSIFY SENT+QUEUED (fire-275). VM: add LightGBM IMMEDIATELY + pickle XGB_brier-0.24987 |
...
|------|------|--------|------------|
| 276 | 2026-06-05T20h | EVEN | Axelrod-verify-FULL-PASS-SHAs-NBA-85e7682e1d(6034L)/POL-6983c86517(4018L)-UNCHANGED; py_compile-OK-both; 14/14-parity-OK; KL-div-OK(compute_consensus_distance-present-both); approx_tokens_d+cost_per_alpha_unit-present-both; Mech-A/B/C+AblationFlags-ALL-CODE-DONE-BLOCKED(do_not_push_hf_space_yet); S18/S22/evo4/evo5/POL-carry-forward-from-fire275; EVEN-WebSearch:arXiv:2606.03600-Set-Preserving-Calibration-CP-P-Values-to-E-Values-Jun2026-NEW(priority=131) |
