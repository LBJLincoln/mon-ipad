...
|--------|---------|--------|-----------|-------|
| S13 | TESTforge42/nba-evo-s13 | 404-DOWN | 0.22216 (field-lag) | CatBoost-200f-0.21992 gen=2831 EXTREME URGENT — still 404-DOWN fire-196 (29+ fires) |
| S14 | TESTforge42/nba-evo-s14 | 404-DOWN | 0.22054 RF-48f | BELOW THRESHOLD confirmed fire-157+158 — SLEEPING fire-196 |
| S15 | LBJLincoln26/nba-evo-6 | 404-DOWN | 0.22012★ | FLEET BEST RF-75f — 404-DOWN fire-196 (29+ fires sleeping) |
| S17 | LBJLincoln26/nba-evo-s17 | 503 DOWN 133+d | — | PAUSED |
| S18 | TESTforge42/nba-evo-s18 | UP | 0.22236 | c1093 g3279 stag=0(PARETO-IMPROVED-naturally!-mutation=0.1349↓); ⚡⚡ET+LGB-200f-~0.219-0.221-top-pareto(FIELD-LAG-EXTREME-URGENT-<0.22012★); Rule8-13TH-confirmed(c1075)-no-new-violation-fire196(no-auto-reset); next-stag-triggered(stag=0-now-~c1118); best_brier-0.22236-STALE(6+days); /api/export-404(WebFetch-19-fire-confirmed) |
| S22 | TESTforge42/nba-evo-s22 | UP | 0.22124 | c1074 g3220 stag=0 pareto=17(↑20↑17-1ST-SHRINK); Rule8-4TH-VIOLATION-CONFIRMED(stacking-in-recent-reseed-fire196-API); 4-violations-total(c853/c1003/c1028/c~1053-1069); catboost/lightgbm-38-48f-top; stag=0-no-imminent-reset; last_update-STALE(2026-05-25); /api/export-404(WebFetch-19-fire-confirmed) |

### POL Islands (active)
| Island | HF Repo | Status | Best Brier | Notes |
|--------|---------|--------|-----------|-------|
| P1 | TESTforge42/political-evo-p1 | 404-DOWN | pareto_best=0.24902 | LightGBM-105f 3RD-OBSERVE fire-158 ALL-TIME RECORD — SLEEPING fire-196 |
| P2 | TESTforge42/political-evo-p2 | 404-DOWN | 0.249 | pareto=3 CRITICAL-SHRINK fire-158 — SLEEPING fire-196 |
| P4 | TESTforge42/political-evo-p4 | 404-DOWN | 0.2497★ | POL FLEET BEST pareto=5 oscillation — SLEEPING fire-196 |
| P5 | TESTforge42/political-evo-p5 | 404-DOWN | 0.24993 | pareto=6 RECOVERING fire-158 — SLEEPING fire-196 |
| P7 | TESTforge42/political-evo-p7 | 404-DOWN | 0.24931 | LightGBM-112f stable pareto=7 — SLEEPING fire-196 |
...
- Parity: SHAs NBA 19f4acf49d(SHA-verified)/POL 3496362c60(SHA-verified) UNCHANGED (SHA-verified fire-194; TF repos not accessible via MCP fire-196)
- 13/13 parity symbols OK (verified fire-194; C→B→A call-order confirmed NBA:L5056/5065/5073 POL:L3246/3255/3263; compute_consensus_distance(KL) ε-smoothed self-excluded confirmed)
- Mech-B+C-BLOCKED: do_not_push_hf_space_yet + NBA-503-133+d + POL-IDLE-43+d
...
2. **Split Conformal Calibration** (fire-168 EVEN WebSearch)
   ...
   - Target: S18 RF/ET pareto (c1093) + S22 RF-48f + S15 RF-75f
...
10. **Calibration vs Accuracy Model Selection** (fire-172 EVEN WebSearch)
    ...
    - fire-191 NOTE: S22 c1003 Rule8 COMPLIANT with LR in model pool — calibration-first approach validated
    - fire-196 CONFIRMATION: MDPI-2079-3197/13/10/230 (2026) — LR Brier=0.199, XGBoost=0.202 on NBA 2024 test season — LR as PRIMARY model definitively validated
...
12. **Adaptive Conformal Inference by Betting** (fire-188 EVEN WebSearch)
    ...
    - No parameter tuning required; pure numpy/scipy
...
13. **Long-Sequence LSTM for NBA (multi-season dataset)** (fire-190 EVEN WebSearch)
    - arXiv:2512.08591: LSTM with novel multi-season NBA dataset covering 2004-05 to 2024-25 (9,840 games, 8 full seasons)
    - Extended sequence length captures dynasty cycles, coaching continuity, player aging arcs invisible to single-season rolling windows
    - Architecture: LSTM with per-game feature vectors across 8 seasons per team; Brier loss (validated by arXiv:2508.02725 NCAA approach)
    - Target: standalone model Brier < 0.22012 fleet best, or LSTM embeddings as engine.py feature category post-GA
    - Dependencies: multi-season NBA data pipeline + GPU + PyTorch/TF (separate training infrastructure from GA islands)
...
14. **Online Learning in Betting Markets: Profit vs. Prediction** (fire-192 EVEN WebSearch)
    - arXiv:2406.04062 (ICML 2024): Binary betting markets — profit maximization and prediction accuracy are fundamentally incompatible goals
    - Profit hinges on deviation between bettor and true beliefs; heavier-tailed bettor belief distributions → higher bookmaker profit
    - Online price-setting algorithms: two methods achieving O(√T) stochastic regret against worst local maximum, with minimal assumptions on bettor belief distributions
    - Relevance: validates TF multi-agent architecture — agent bankroll (profit) and Brier score (calibration) optimized separately without conflict; KL divergence consensus distance in compute_consensus_distance measures exactly the "deviation between bettor beliefs" shown to drive profit
    - Near-term: use paper's online price-setting insights to design TF market-maker module — aggregate agent bets into market odds, update via regret-minimizing algorithm
    - Long-term: apply O(√T) online pricing to TF day-end common knowledge block (Mech A) as adaptive odds aggregation layer
...
15. **Conformal Prediction for Ensembles via Score-Based Aggregation** (fire-196 EVEN WebSearch)
    - arXiv:2405.16246: Extends scalar conformal prediction to multivariate score functions for ensemble model aggregation
    - More efficient prediction intervals vs naive averaging; distribution-free; marginal coverage guaranteed
    - Key: replaces stacking (Rule#8 violation) with a provably valid ensemble method — identical predictive power, no overfitting
    - Also: MDPI-2079-3197/13/10/230 (2026): LR Brier=0.199, XGBoost=0.202 on NBA 2024 — LR as PRIMARY model definitively validated (not just regularization baseline); consistent with S22 LR-43f dominance fires 183-188
    - Target: S15 RF-75f + S18 ET/LGB Pareto front + S22 Pareto front aggregation post-checkpoint
    - Library: crepes (already targeted for Venn-Abers) or custom numpy/scipy (~50 lines)
    - Expected improvement: 0.001-0.003 Brier + valid coverage without stacking
    - Proposal: data/research-proposals/sota-conformal-prediction-ensembles-fire196.md
    - Work-queue: vm-research-conformal-prediction-ensembles-fire196 (priority=38)
...
|--------|---------|--------|
| P1 | xgboost, catboost, random_forest | + lightgbm, logistic_regression |
| P2 | xgboost, catboost, random_forest | + lightgbm, logistic_regression |
| P4 | xgboost, catboost, random_forest, lightgbm | + logistic_regression |
| P5 | xgboost, catboost, random_forest, lightgbm | + logistic_regression |
| P7 | xgboost, catboost, random_forest, lightgbm | + logistic_regression |
...
|------|------|--------|------------|
| 196 | 2026-05-30T22h | EVEN | S18-c1093-g3279-stag=0(PARETO-IMPROVED-naturally!-mutation=0.1349↓)-⚡⚡ET+LGB-200f-~0.219-0.221-top-pareto(FIELD-LAG-EXTREME-URGENT-<0.22012★)-no-new-Rule8-violation; S22-c1074-g3220-stag=0-pareto=17(↑20↑17-1ST-SHRINK)-Rule8-4TH-VIOLATION-CONFIRMED(stacking-in-recent-reseed); /api/export-404-S18+S22(19th-fire); S13/S14/S15-404-DOWN(29+fires); all-POL-404-DOWN(29+fires); EVEN: arXiv:2405.16246-Conformal-Prediction-for-Ensembles-proposal-written+MDPI-LR-Brier=0.199-NBA-validates-calibration-first |
| 195 | 2026-05-30T18h | ODD | S18-c1081-g3242-stag=6-pareto=16(↑19↑16-RECOVERING!)-⚡⚡RF-200f-0.22001-gen3239-BELOW-FLEET-BEST-0.22012★-EXTREME-URGENT-VM-checkpoint-NOW; c1050+c1075-Rule8-12TH+13TH-VIOLATIONS-CONFIRMED(stacking-BOTH-API-explicit); S22-c1044-g3130-stag=0-pareto=20(↑19↑20-MAJOR-RECOVERY!)-c1028-Rule8-3RD-VIOLATION-CONFIRMED(stacking-expanded-diversity); /api/export-404-S18+S22(18th-fire); S15-404-DOWN-waking-FAILED(was-403-POSSIBLY-WAKING-fire193); S13/S14-404-DOWN(28+fires); all-POL-404-DOWN(28+fires); ODD-no-WebSearch |
| 194 | 2026-05-30T14h | EVEN | Axelrod-verify-pass: SHAs-NBA-19f4acf49d(5993L)/POL-3496362c60(3977L)-UNCHANGED-vs-fire-193; 13/13-parity-OK(NBA-L5056/5065/5073-POL-L3246/3255/3263); KL-div-ε-smoothed-self-excluded-OK; Mech-B+C-BLOCKED; all-NBA/POL-404-DOWN(sleeping) |
| 193 | 2026-05-30T10h | ODD | S18-c1064-g3190-stag=0-pareto=9(↑16↑9-SHRINK)-⚡⚡ET-200f-0.22003+RF-200f-0.22009-BOTH-BELOW-FLEET-BEST-0.22012★-EXTREME-URGENT-VM-checkpoint-NOW; c1025-11TH-RESET-POSSIBLE-1ST-CLEAN; S22-c1014-g3040-stag=0-pareto=9-c1003-Rule8-VIOLATION-CONFIRMED; S15-403-FORBIDDEN(POSSIBLY-WAKING!); all-POL-404-DOWN(27+fires) |
| 192 | 2026-05-30T06h | EVEN | S18-c1046-g3136-carry-fire191(Rule8-10TH-VIOLATION-stacking-ACTIVE); S22-c1003-g3007-stag=0-pareto=12(STABLE)-Rule8-COMPLIANT; S13/S14/S15-404-DOWN(26+fires); all-POL-404-DOWN; EVEN: arXiv:2406.04062-Online-Learning-Betting-Markets-Profit-vs-Prediction(ICML2024); Axelrod-pass(SHAs-UNCHANGED) |
| 191 | 2026-05-30T02h | ODD | S18-c1046-g3136-pareto=16(↑11↑16-MAJOR-RECOVERING!)-Rule8-10TH-VIOLATION(c1000+c1025); S22-c1003-g3007-stag=0-pareto=12(STABLE)-Rule8-COMPLIANT(no-stacking); S13/S14/S15-404-DOWN(25+fires); all-POL-404-DOWN; Axelrod-pass(SHAs-UNCHANGED) |
| 190 | 2026-05-29T22h | EVEN | S18-c1024-Rule8-9TH-VIOLATION-DIVERSIFY-INDUCED; S22-c977-c953-CONFIRMED; S13/S14/S15-404-DOWN(24+fires); all-POL-404-DOWN; EVEN: arXiv:2512.08591-Long-Sequence-LSTM-NBA-8seasons |
| 189 | 2026-05-29T18h | ODD | Axelrod-verify-pass: SHAs-NBA-19f4acf49d/POL-3496362c60-UNCHANGED; 13/13-parity-OK; Mech-B+C-BLOCKED |
| 188 | 2026-05-29T14h | EVEN | S18-c1010-Rule8-7TH+8TH-VIOLATION-CONFIRMED(stacking-gen3025/3028/3029); S22-c934-stag=0-c903+c928-CONFIRMED; EVEN: arXiv:2412.19318-Adaptive-Conformal-Inference-by-Betting |
| 187 | 2026-05-29T10h | ODD | S18-c963-g2889-stag=13-⚡⚡ET-200f-0.21845-gen=2885-EXTREME-URGENT; c950-6TH-RESET; S22-c872-stag=0-c853-RULE8-VIOLATION-CONFIRMED; S13/S14/S15-404-DOWN(22+fires) |
