...
| S18 | TESTforge42/nba-evo-s18 | EVOLVING | 0.22326 | **fire-268**: c=832 g=2496 stag=15 pareto=14. **⚡⚡CatBoost-200f-0.2191 RANK-2 (8.2bp below 0.22012!) + CatBoost-0.2193 RANK-3. ET-0.21938 CONFIRMED EVICTED at 34TH RESET c817.** RULE8-VIOLATION: stacking-45f RANK-0 (brier-trend=0.24738 flat 43gen). 34TH-RESET-c817-CONFIRMED(c792+25=c817-EXACT). stag=15 BUILDING (diversify threshold!). **35TH RESET ~c842 (10 cycles — IMMINENT!)**. VM: (1) SAVE CatBoost-0.2191 pickle BEFORE ~c842, (2) REMOVE stacking NOW (stag=15 means diversify approaching — checkpoint FIRST). |
| S22 | TESTforge42/nba-evo-s22 | EVOLVING | 0.2207 | **fire-268**: c=223 g=667 stag=0 pareto=12. **⚡⚡XGBoost-200f-0.21985 RANK-1 (2.7bp below 0.22012!). RF-0.21903 CONFIRMED EVICTED at 8TH RESET c203.** RULE8-VIOLATION: stacking in model_types. 8TH-RESET-c203-CONFIRMED(c178+25=c203-EXACT). /api/export-403 (try authenticated). **9TH RESET ~c228 IMMINENT (5 cycles! CRITICAL!)**. VM: (1) SAVE XGBoost-0.21985 pickle IMMEDIATELY before ~c228, (2) REMOVE stacking NOW — pattern: stacking evicts every best candidate at reset. |
...
| P5 | lbjlincoln/political-alpha-5 | EVOLVING | 0.25036 | **fire-268**: c=502 stag=0. **RULE9 RESOLVED!** LightGBM DOMINANT: top-3 ALL LightGBM (LGB-76f/94f/97f, brier=0.25036). Fresh from FRESH RESTART at fire-265 (c=3) → now c=502. Cycle speed ~1.5min/cycle confirmed. VM: no action needed — P5 healthy. |
| P7 | lbjlincoln/political-alpha-7 | EVOLVING | 0.25331 | **fire-268**: c=3225 stag=0 pareto=17. **RULE9 LIKELY RESOLVED**: LightGBM now visible in top-5 model_types (vs. fire-263 where only xgboost+extra_trees confirmed). VM: verify LightGBM is definitively in MODEL_TYPES config to close violation formally. |
...
44. **Parameter-Free and Group Conditional Online Conformal Prediction** (fire-268 EVEN WebSearch)
    - arXiv:2606.00419 (Jun 2026): "Parameter-Free and Group Conditional Online Conformal Prediction"
    - Key finding: Parameter-free algorithm achieving simultaneous group-conditional coverage without prior knowledge of group structure or hyperparameter tuning. Extends ACI by removing the need to specify learning rate α via multiplicative-weights adaptation over group coverage gaps.
    - Application 1: NBA group-conditional CP without prespecifying groups (venue × back-to-back × fatigue × season_phase) — ~60 lines in calibration/isotonic_calibrator.py
    - Application 2: Add `group_coverage_gap` as 9th Pareto objective (discriminates models that calibrate well on average vs. across all game types)
    - Application 3: Universal calibrator across all islands (no per-island hyperparameter tuning needed)
    - Application 4: Port to political_engine.py — groups: state_type × incumbency × competitive_tier × cycle_type
    - Application 5: Add `group_conditional_coverage` dict to /api/export
    - Library: MAPIE + custom multiplicative-weights adapter (~145 lines total, no new deps)
    - Expected improvement: 0.001-0.002 Brier + simultaneous group-conditional coverage guarantee
    - Proposal: data/research-proposals/sota-parameter-free-group-conditional-ocp-fire268.md
    - Work-queue: vm-research-parameter-free-group-ocp-fire268 (priority=123)
45. **Post-Processing CP Approach for Conditional Coverage via Pivotal Scores** (fire-268 EVEN WebSearch)
    - arXiv:2605.25852 (May 2026): "A Post-Processing CP Approach for Conditional Coverage via Pivotal Scores"
    - Key finding: Post-processing calibration using pivotal (distribution-free) nonconformity scores achieves conditional coverage without retraining — works as wrapper around any pre-trained model. Directly applicable to S18 CatBoost-0.2191 and S22 XGBoost-0.21985 before production promotion.
    - Application 1: Wrap S18/S22/evo4/evo5 pareto best models with PivotalScoreCP post-processor for conditional coverage
    - Application 2: Replace isotonic calibration in engine.py with pivotal score CP as optional drop-in replacement
    - Application 3: Add `pivotal_coverage_gap` calibration quality metric to /api/export
    - Application 4: Add conditional coverage gate to /api/checkpoint promotion logic (pivotal_coverage_gap < 0.05)
    - Application 5: Port to political_engine.py for P4/P5/P7 state-level conditional coverage
    - Library: scipy.stats (already available) + custom PivotalScoreCP class (~170 lines, no new deps)
    - Expected improvement: 0.001-0.003 Brier + conditional coverage for extreme game conditions
    - Proposal: data/research-proposals/sota-postprocessing-cp-conditional-coverage-fire268.md
    - Work-queue: vm-research-postprocessing-cp-conditional-fire268 (priority=124)
...
|--------|---------|--------|
| P1 | extra_trees, xgboost_brier, lightgbm_brier, lightgbm | ✅ RULE9 RESOLVED fire-256 — 404-DOWN 87+ fires |
| P2 | lightgbm is best (xgboost, lightgbm, catboost, xgboost_brier) | ✅ RESOLVED fire-255 — 404-DOWN 87+ fires |
| P4 | logistic_regression, xgboost_brier, catboost (c2169) — NO lightgbm | ⚠️ VIOLATION PERSISTS fire-268 — EVOLVING (VM: add LightGBM IMMEDIATELY) |
| P5 | lightgbm DOMINANT (LGB-76f/94f/97f top-3, brier=0.25036) | ✅ RULE9 RESOLVED fire-268 — c=502 EVOLVING healthy |
| P7 | lightgbm in top-5 model_types (c3225) | ✅ LIKELY RESOLVED fire-268 — VM: confirm MODEL_TYPES config has LightGBM |
...
|------|------|--------|------------|
| 268 | 2026-06-04T12h | EVEN | TRIPLE-FLEET-BEST-BREAKTHROUGH: S18-CatBoost-0.2191(8.2bp-below!-stag15-34TH-RESET-c817-CONFIRMED-ET-EVICTED-35TH~c842-10cy)+S22-XGBoost-0.21985(2.7bp-below!-8TH-RESET-c203-CONFIRMED-RF-EVICTED-9TH~c228-5cy!-CRITICAL)+evo4-RF-0.21985(2.7bp-below!-35TH/36TH/37TH-RESETS-38TH~c1064-5cy!-CRITICAL)-ALL-RULE8-stacking!; evo5-c1426-34TH-RESET-HAPPENING-NOW-rank1=0.22042-BELOW-GATE; P5-RULE9-RESOLVED!(LGB-top3-c502); P7-RULE9-LIKELY-RESOLVED(LGB-top5-c3225); P4-RULE9-PERSISTS(c2169); EVEN:arXiv:2606.00419(priority=123)+arXiv:2605.25852(priority=124); Axelrod-carry-forward-SHAs-UNCHANGED |
