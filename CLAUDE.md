...
| S15 | Nomos42/nba-evo-6 | EVOLVING | 0.22342 | **fire-263**: BACK UP 3RD TIME! c=1153 g=3458 stag=0 pareto=13. **33RD-RESET-c1151-CONFIRMED** (stagnation 25cy hard-reset). RULE8-CLEAN: reseed=[xgb_brier,lgbm,lgbm_brier,cat,et] — NO stacking! RF-62f best. Next reset ~c1176. VM: (1) verify MODEL_TYPES excludes stacking, (2) implement /api/export URGENTLY. |
| S17 | LBJLincoln26/nba-evo-s17 | 503 DOWN 144+d | — | PAUSED |
| S18 | TESTforge42/nba-evo-s18 | EVOLVING | 0.22326 | **fire-271**: c=~889 g=~2667 stag=0. **36TH-RESET-c867-CONFIRMED(c842+25=c867-EXACT)**. **37TH-RESET-~c892-IMMINENT (3cy!)**. RULE8-VIOLATION: stacking-37f PERSISTS — remote diversify c848 CONFIRMED INEFFECTIVE (VM-only app.py fix required). best_brier=0.22326 (field-lag). /api/export-404(CRITICAL). VM: FIX app.py to REMOVE stacking from MODEL_TYPES IMMEDIATELY before ~c892. |
| S22 | TESTforge42/nba-evo-s22 | EVOLVING | 0.2207 | **fire-271**: c=~253 g=~759 stag=0. **10TH-RESET-c253-HAPPENING-NOW(c228+25=c253-EXACT)**. Pre-reset CLEAN REBUILD: lightgbm_brier-44f dominant (NO stacking! gen691-698+). pareto7→17 GROWING. Post-reset RULE8 fate CRITICAL — next reset ~c278 (25cy). /api/export-404(CRITICAL). VM: check /api/status post-c253 IMMEDIATELY — pareto growth = CLEAN, pareto collapse = stacking re-contaminated. REMOVE stacking from MODEL_TYPES before next reset. |
...
### MODEL_TYPES Status
| Island | Current (fire-268) | Status |
|--------|---------|--------|
| P1 | extra_trees, xgboost_brier, lightgbm_brier, lightgbm | ✅ RULE9 RESOLVED fire-256 — 404-DOWN 92+ fires |
| P2 | lightgbm is best (xgboost, lightgbm, catboost, xgboost_brier) | ✅ RESOLVED fire-255 — 404-DOWN 92+ fires |
| P4 | xgboost_brier best (c2169) — NO lightgbm | ⚠️ VIOLATION PERSISTS fire-268 — EVOLVING (VM: add LightGBM IMMEDIATELY) |
| P5 | lightgbm dominant (top-3 all LGB at c502) | ✅ RULE9 RESOLVED fire-268! 0.24994 approaching POL gate 0.2497 |
| P7 | lightgbm now in top-5 (c3225) | ✅ LIKELY RESOLVED fire-268 — monitor for full confirmation |
...
|------|------|--------|------------|
| 272 | 2026-06-05T04h | EVEN | Axelrod-verify-FULL-PASS-SHAs-NBA-85e7682e1d(6034L)/POL-6983c86517(4018L)-UNCHANGED; py_compile-OK-both; 11/11-parity-OK; KL-div-OK(epsilon-smoothed-compute_consensus_distance-present-both); Mech-C-line5092(NBA)/3282(POL)+Mech-B-line5100(NBA)/3290(POL)+Mech-A-line5111(NBA)/3301(POL)-CONFIRMED; order-C→B→A-CORRECT; approx_tokens_d+cost_per_alpha_unit-present-both; AXELROD_ABLATION-5-configs-confirmed-both; do_not_push_hf_space_yet-MAINTAINED; S18/S22/evo4/evo5+POL-carry-forward-from-fire271(no-API-access-from-cloud); EVEN-WebSearch-SKIPPED(Axelrod-verify-session) |
