...
LOG="/home/lahargnedebartoli/mon-ipad/logs/watchdog.log"
...
if ! pgrep -f "nomos42_brain.py" > /dev/null 2>&1; then
    ...
    cd /home/lahargnedebartoli/mon-ipad && bash scripts/telegram/start_bots.sh start >> "$LOG" 2>&1
    FIXES=$((FIXES + 1))
...
if ! pgrep -f "rgwa_bot.py" > /dev/null 2>&1; then
    ...
    cd /home/lahargnedebartoli/rgwa && bash scripts/telegram/start_bot.sh start >> "$LOG" 2>&1
    FIXES=$((FIXES + 1))
...
if ! { pgrep -f "nba-data-server" > /dev/null 2>&1 || pgrep -f "http\.server 8080" > /dev/null 2>&1; }; then
    ...
    nohup python3 -m http.server 8080 -b 0.0.0.0 --directory /home/lahargnedebartoli/mon-ipad/data > /dev/null 2>&1 &
    FIXES=$((FIXES + 1))
...
if ! pgrep -f "terminal_api.py" > /dev/null 2>&1; then
    ...
    source /home/lahargnedebartoli/mon-ipad/.env.local 2>/dev/null
    nohup python3 /home/lahargnedebartoli/mon-ipad/scripts/terminal_api.py > /tmp/terminal_api.log 2>&1 &
    FIXES=$((FIXES + 1))
...
if [ "$DISK_PCT" -gt 90 ]; then
    ...
    # Auto-clean old logs
    find /home/lahargnedebartoli/mon-ipad/logs/ -name "*.log" -size +50M -exec truncate -s 10M {} \;
    find /tmp/ -name "watchdog-alert-cooldown-*" -mtime +1 -delete 2>/dev/null
fi

# ── 6. Log rotation (keep logs under 20MB each) ──────────────
for logfile in /home/lahargnedebartoli/mon-ipad/logs/*.log /tmp/nomos42-brain.log /tmp/rgwa-bot.log; do
    if [ -f "$logfile" ]; then
