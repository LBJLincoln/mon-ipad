...
# Axelrod ablation config — arXiv:2605.03310 §3 maps paper's 5 coordination configs to A/B/C flags.
# Set AXELROD_ABLATION env var: "A+B+C" (full, default), "A+C", "B+C", "C", "" (no-CK baseline).
_AXELROD_ABLATION_CFG = os.environ.get("AXELROD_ABLATION", "A+B+C").upper()
AXELROD_MECH_A_ON: bool = "A" in _AXELROD_ABLATION_CFG   # day-end CK broadcast
AXELROD_MECH_B_ON: bool = "B" in _AXELROD_ABLATION_CFG   # sacrificial role reallocation
AXELROD_MECH_C_ON: bool = "C" in _AXELROD_ABLATION_CFG   # post-mortem log writes
print(f"[axelrod-ablation] POL cfg={_AXELROD_ABLATION_CFG!r} A={AXELROD_MECH_A_ON} B={AXELROD_MECH_B_ON} C={AXELROD_MECH_C_ON}")
...
# Axelrod Mech B — one-sentence directive for each archetype (political-alpha flavour)
AXELROD_ARCHETYPE_DESCRIPTIONS: Dict[str, str] = {
    "political_sentiment": "Trade only on sentiment shifts: poll swings, approval-rating moves, or social-media velocity spikes.",
    "insider_tracking": "Mirror Form 4 SEC insider trades filed in the last 48 h; prioritise cluster buying over isolated filings.",
    "trump_volatility": "Trade sectors with highest Trump-policy sensitivity (tariffs, energy, immigration); buy vol on policy news.",
    "foreign_sovereign_flow": "Follow sovereign wealth fund and foreign central bank 13F filings; mirror their largest new positions.",
    "regulatory_arb": "Long sectors facing regulatory rollback announcements; short sectors hit by new enforcement actions.",
    "congress_trading_mirror": "Mirror STOCK Act disclosures filed by both chambers within the past 7 days; prioritise cross-party consensus.",
    "macro_narrative": "Trade the dominant macro narrative (rate cuts, inflation, recession) as expressed in sector ETF flows.",
    "crisis_contrarian": "Buy sectors that sold off 5 %+ in a geopolitical crisis week; mean-revert after peak fear.",
    "election_cycle_timing": "Allocate to historically election-sensitive sectors based on polling trajectory and election-cycle timing.",
    "fed_watcher": "Trade rate-sensitive sectors (XLF/XLRE/XLU) solely on Fed speaker comments and FOMC positioning shifts.",
    "geopolitical_risk": "Buy defense/energy on geopolitical escalation; fade safe-haven flows on de-escalation.",
    "congressional_calendar": "Trade sectors with bills on committee schedule this week; buy before markup hearings, sell after.",
    "lobbying_flow": "Long sectors with top-10 lobbying spend increases quarter-over-quarter; short those with lobbying cuts.",
    "pac_money_velocity": "Trade sectors where PAC contribution velocity is accelerating fastest in the latest FEC 48 h filings.",
    "treasury_curve_divergence": "Trade steepener/flattener plays via XLF vs XLRE based on 2y-10y curve moves.",
    "commodities_war_premium": "Long energy/materials when war premium appears under-priced vs CDS spreads; fade when over-priced.",
    "dollar_strength_fade": "Short multinationals (XLK/XLC) in strong-dollar regimes; long domestic defensives.",
    "emerging_market_risk": "Buy USD-hedged domestic defensives when EM credit spreads widen; unwind on EM rally.",
    "defense_budget_catalyst": "Long XLI on Pentagon budget proposal weeks; short duration around budget resolution deadlines.",
    "sanctions_arbitrage": "Long domestic energy/materials alternatives when new sanctions block foreign supply; fade on reversal.",
}
...
{
  "day_strategy": "MUST start with STRUCTURAL DIVERGE [peer] | STRUCTURAL COMPLEMENT [peer] | CONSENSUS_AGREE_JUSTIFIED [peer] (reason=<structural_reason>) citing your REASONING TEMPLATE — then 1-2 sentences on approach",
  "coalition_proposal": {
  ...
  "ck_consensus_stance": {
    ...
    "reason": "1 sentence citing specific peers/positions from COMMON_KNOWLEDGE"
  },
  "allocations": [
    {
      "event_idx": 1,
...
- day_strategy: MANDATORY anti-groupthink gate (Axelrod Mech A). If CONSENSUS_AGREE_JUSTIFIED, add (reason=<why agreement is not imitation — cite structural difference in your REASONING TEMPLATE>). Councils score day_strategy for DMAD compliance; empty/missing = fail.
...
def build_common_knowledge_block(day_date: str, state: Dict, agent_logs: Dict,
                                  ...
                                  day_idx: int = 0,
                                  next_day_sacrificial: Optional[Dict[str, str]] = None,
                                  next_day_challenge: Optional[Dict[str, int]] = None) -> str:
    ...
    for rank, (tid, ts) in enumerate(ranked, 1):
        ...
        if ts["bankroll"] < 50:
            ...
            role = " [TOP-3]"
        _td7 = compute_trailing_delta(tid, state, agent_logs, 7)
        lines.append(
            f"  #{rank} {cfg.get('name', tid):<20} ${ts['bankroll']:.2f} GF={gf:.3f}× ({roi:+.1f}%)"
            f" | 7dΔ{_td7:+.1f} | {ts['total_bets']}b {wr:.0f}%WR | DD {ts['max_drawdown']:.1%}{role}"
        )
    ...
    if recent_dates:
        ...
        _stake_ctr: Dict[str, float] = {}
        _pnl_ctr: Dict[str, float] = {}
        _win_ctr: Dict[str, int] = {}
        _n_active = 0
        for _t2, _ in ranked:
            ...
            if _d2:
                ...
                for _a2 in _d2.get("allocations", []):
                    ...
                    _stake_ctr[_pk] = _stake_ctr.get(_pk, 0) + float(_a2.get('stake', 0) or 0)
                    _pnl_ctr[_pk] = _pnl_ctr.get(_pk, 0.0) + float(_a2.get('profit', 0) or 0)
                    _win_ctr[_pk] = _win_ctr.get(_pk, 0) + (1 if _a2.get('won') else 0)
        if _pick_ctr:
        ...
        # Closes the meta-learning loop — agents see HOW peers responded to CK[D-1].
        _ck_stances = []
        for _t3, _ in ranked:
            _d3 = next((l for l in reversed(agent_logs.get(_t3, [])) if l.get("date") == _yesterday), None)
            if _d3:
                _st = (_d3.get("ck_consensus_stance") or {})
                if _st and _st.get("stance"):
                    _cfg3 = TRADERS.get(_t3, {})
                    _ck_stances.append(
                        f"  {_cfg3.get('name', _t3):<20} [{_st['stance'].upper()}] {(_st.get('reason') or '')[:80]}"
                    )
        if _ck_stances:
    ...
    # Axelrod-2026 fire-96: surface day D+1 tier assignments in CK[D] so every agent
    # sees who is constrained tomorrow — enables targeted diverge/agree decisions.
    if next_day_sacrificial or next_day_challenge:
        _ranked_tiers = sorted(state.items(), key=lambda x: -x[1]["bankroll"])
        _top_tids = {_t for _t, _ in _ranked_tiers[:3]}
        lines.append("\nTOMORROW\'S SOCIETY TIERS (day D+1 role assignments — visible to all agents now):")
        for _tr, (_tt, _) in enumerate(_ranked_tiers, 1):
            _tcfg = TRADERS.get(_tt, {})
            _tname = _tcfg.get("name", _tt)
            ...
            elif next_day_challenge and _tt in next_day_challenge:
                lines.append(f"  CHALLENGE  #{_tr}: {_tname} — must state explicit improvement plan")
            elif _tt in _top_tids:
                lines.append(f"  TOP-3      #{_tr}: {_tname} — unchanged prompt (proven strategy)")
        lines.append("")
...
def build_sacrificial_system_suffix(archetype: str) -> str:
    ...
    description = AXELROD_ARCHETYPE_DESCRIPTIONS.get(
        archetype, "Apply this archetype consistently to every trade today."
    )
    return (
        ...
        f"ARCHETYPE DIRECTIVE: {description}\n"
    )
...
def compute_consensus_distance(tid: str, day_date: str, state: Dict, agent_logs: Dict) -> float:
    """Axelrod Mech C: KL divergence D_KL(agent || peers) over ticker/direction bet distribution.
    ...
    for other_tid, logs in agent_logs.items():
        ...
        for a in day_log.get("allocations", []):
            ...
            if other_tid == tid:
                ...
                society[tick] += 1  # peers only: exclude self for true D_KL(agent||peers)
...
def _parse_dmad_prefix(strategy: str) -> str:
    """Axelrod Mech C fire-85: classify DMAD compliance label for paper dataset."""
    s = (strategy or "").strip().upper()
    if s.startswith("STRUCTURAL DIVERGE") or s.startswith("STRUCTURAL_DIVERGE"):
        return "STRUCTURAL_DIVERGE"
    if s.startswith("STRUCTURAL COMPLEMENT") or s.startswith("STRUCTURAL_COMPLEMENT"):
        return "STRUCTURAL_COMPLEMENT"
    if s.startswith("CONSENSUS_AGREE_JUSTIFIED"):
        return "CONSENSUS_AGREE_JUSTIFIED"
    if s.startswith("CHALLENGE_RESPONSE"):
        return "CHALLENGE_RESPONSE"
    if s.startswith("ARCHETYPE["):
        return "ARCHETYPE_SACRIFICIAL"
    return "UNCLASSIFIED"


def _compute_murphy_resolution(decisions: list) -> float:
    """Murphy (1973) resolution: E[p(1-p)] over confidence values.

    Higher = agent moves further from base rate = more discriminating.
    arXiv:2605.03310 §3.2 Murphy calibration signature for MAS prediction markets.
    """
    confs = [float(d.get("confidence", 0.5) or 0.5) for d in decisions]
    if not confs:
        return 0.0
    return round(sum(c * (1 - c) for c in confs) / len(confs), 6)


def _compute_brier_day(decisions: list) -> float:
    """Day-level Brier score: mean((confidence - outcome)^2) for this agent."""
    pairs = [(float(d.get("confidence", 0.5) or 0.5), int(bool(d.get("won"))))
             for d in decisions]
    if not pairs:
        return 0.0
...
def write_axelrod_log(day_idx: int, day_date: str, state: Dict,
                       ...
                       challenge_map: Optional[Dict[str, int]] = None,
                       society_archetypes_by_day: Optional[Dict] = None,
    ...
    try:
        ...
        rows = []
        # Axelrod Mech C: fleet-wide consensus pick distribution (how many agents picked each ticker)
        _cpd: Dict[str, int] = {}
        for _tid2, _ts2 in state.items():
            _logs2 = agent_logs.get(_tid2, [])
            _dlog2 = next((l for l in reversed(_logs2) if l.get("date") == day_date), None)
            for _d2 in (_dlog2.get("allocations", []) if _dlog2 else []):
                _t2 = _d2.get("ticker", "")
                if _t2:
                    _cpd[_t2] = _cpd.get(_t2, 0) + 1
        # Axelrod Mech C: trailing 7-day society-wide archetype set (Mech B dedup audit trail)
        _today_dt = datetime.strptime(day_date, "%Y-%m-%d")
        _used_7d: set = set()
        for _d7, _a7 in (society_archetypes_by_day or {}).items():
            try:
                _delta = (_today_dt - datetime.strptime(_d7, "%Y-%m-%d")).days
                if 0 < _delta <= 7:
                    _used_7d.update(_a7)
            except (ValueError, TypeError):
                pass
        used_archetypes_7d = sorted(_used_7d)
        for tid, ts in state.items():
            logs = agent_logs.get(tid, [])
            day_log = next((l for l in reversed(logs) if l.get("date") == day_date), None)
            ...
            rows.append({
                ...
                "tier": (
                    "sacrificial" if tid in sacrificial_map
                    else "challenge" if (challenge_map and tid in challenge_map)
                    else "top"
                ),
                ...
                "decisions_summary": [
                    {
                        ...
                        "confidence": round(d.get("confidence", 0.5), 4),
                        "thesis": d.get("thesis", ""),
                        "won": bool(d.get("won")),
                ...
                "coalition_proposal": (day_log.get("coalition_proposal") or {}) if day_log else {},
                "council_alignment": (day_log.get("council_alignment") or {}) if day_log else {},
                "bankroll_growth_factor": round(ts["bankroll"] / 100.0, 6),
                "consensus_pick_distribution": {t: _cpd[t] for t in {d.get("ticker", "") for d in decisions if d.get("ticker")}},
                "fleet": "political",
                ...
                "used_archetypes_7d": used_archetypes_7d,
                "is_rogue": (day_rogue_state or {}).get(tid, {}).get("is_rogue", False),
                "rogue_reasons": (day_rogue_state or {}).get(tid, {}).get("reasons", []),
                "murphy_score_d": _compute_murphy_resolution(decisions),
                "brier_score_d": _compute_brier_day(decisions),
                "cost_per_alpha_unit": round(
                    (day_log.get("approx_tokens_d") or 0) / max(
                        abs(sum(d.get("profit", 0) for d in decisions)), 0.01
                    ), 4
...
def run_experiment(progress=gr.Progress(track_tqdm=False)):
    ...
    # Axelrod Mech B: restore society-wide archetype history + next-day sacrificial map
    if saved:
        for _d, _archs in saved.get("society_archetypes_by_day", {}).items():
            _society_archetypes_by_day[_d] = set(_archs)
        if saved.get("sacrificial_assignments"):
            _sacrificial_assignments.update(saved["sacrificial_assignments"])
        if saved.get("challenge_assignments"):
            _challenge_assignments.update(saved["challenge_assignments"])
    ...
    for day_idx, day_date in enumerate(dates_sorted):
        ...
        for tid, cfg in TRADERS.items():
            ...
            day_log = {
                ...
                "approx_tokens_d": (len(system_prompt) + len(user_prompt) + len(raw_response or "")) // 4,
            }
            ...
            if parsed and parsed.get("allocations"):
                ...
                for alloc in parsed["allocations"]:
                    ...
                    day_log["allocations"].append({
                        ...
                        "edge": round(float(alloc.get("edge", 0) or 0), 4),
        ...
        if AXELROD_MECH_C_ON:
            write_axelrod_log(day_idx, day_date, state, dict(_agent_logs), dict(_sacrificial_assignments),
                              challenge_map=dict(_challenge_assignments),
                              society_archetypes_by_day=dict(_society_archetypes_by_day),
                              day_rogue_state=day_rogue_state)

        # Axelrod Mechanism B: compute sacrificial + challenge assignments for NEXT day
        # Done BEFORE CK build so CK[D] surfaces tomorrow's tiers to every agent.
        if AXELROD_MECH_B_ON:
        ...
        # Axelrod Mechanism A: build COMMON_KNOWLEDGE[D] — now includes D+1 tier assignments
        if AXELROD_MECH_A_ON:
            prev_day_ck = build_common_knowledge_block(
                ...
                day_idx=day_idx,
                next_day_sacrificial=dict(_sacrificial_assignments),
                next_day_challenge=dict(_challenge_assignments),
            )
        ...
        with _state_lock:
            _experiment_state = {
                ...
                "society_archetypes_by_day": {d: sorted(archs) for d, archs in _society_archetypes_by_day.items()},  # Axelrod Mech B
                "sacrificial_assignments": dict(_sacrificial_assignments),  # Axelrod Mech B
                "challenge_assignments": dict(_challenge_assignments),  # Axelrod Mech B
                # Collective experiment (2026-04-17)
                "season_target": SEASON_TARGET,
