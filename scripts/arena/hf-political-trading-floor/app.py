...
def write_axelrod_log(day_idx: int, day_date: str, state: Dict,
    ...
    try:
        ...
        # Axelrod Mech B spec: society-wide USED_ARCHETYPES in trailing 7 days
        try:
            from datetime import timedelta as _td2
            _cutoff7 = datetime.fromisoformat(day_date) - _td2(days=7)
            _used_set7: set = set()
            for _d7, _archs7 in (society_archetypes_by_day or {}).items():
                try:
                    if datetime.fromisoformat(_d7) >= _cutoff7:
                        _used_set7.update(_archs7)
                except ValueError:
                    pass
            _used_archetypes_7d = sorted(_used_set7)
        except Exception:
            _used_archetypes_7d = []
        for tid, ts in state.items():
            logs = agent_logs.get(tid, [])
            ...
            rows.append({
                ...
                "used_archetypes_7d": _used_archetypes_7d,
            })
