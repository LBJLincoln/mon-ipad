...
def _call_agent(persona: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 2026-04-30 — TAURICRESEARCH BULL/BEAR/SYNTHESIS DEBATE for ITF.
    # Each agent runs 3 LLM calls per tick (was 1). Bull/bear use a cheap
    # debate provider; synthesis uses the agent's primary model.
    debate_provider = os.environ.get("TAURIC_DEBATE_PROVIDER", "cerebras:llama3.1-8b")
    bull_text = ""
    bear_text = ""
    ...
    try:
        bull_msgs = [
            {"role": "system", "content": sys_content +
             ...
             "Name ONE strong bullish thesis right now (BULL_NDX, BULL_TQQQ, "
             "LONG_NVDA, BULL_BTC, etc). Cite specific data: tape color, vol "
             "regime, recent move, sector leadership, news catalyst. Output "
             "2-4 sentences, plain text. NO JSON, NO trade list."},
            {"role": "user", "content": prompt},
        ]
        b = gateway_call(debate_provider, bull_msgs, temperature=0.6, max_tokens=180,
                         fallback_direct=False, timeout=_CALL_AGENT_TIMEOUT)
        bull_text = ((b or {}).get("text") or "").strip()[:600]
    except Exception:
        pass

    # PASS 2 — BEAR (sees bull text)
    try:
        bear_msgs = [
            {"role": "system", "content": sys_content +
             ...
             "Counter-argue. Find structural weakness (correlation, IV crush, "
             "public-side trap, regime mismatch, theta decay). 2-4 sentences."},
            {"role": "user", "content": prompt},
        ]
        b2 = gateway_call(debate_provider, bear_msgs, temperature=0.6, max_tokens=180,
                          fallback_direct=False, timeout=_CALL_AGENT_TIMEOUT)
        bear_text = ((b2 or {}).get("text") or "").strip()[:600]
    except Exception:
        pass

    # PASS 3 — TRADER SYNTHESIS (returns JSON)
    synth_sys = (
        sys_content +
        "\n\n=== TAURIC PASS 3/3: TRADER (FINAL DECISION) ===\n"
        f"---BULL---\n{bull_text or '(bull failed)'}\n---END---\n"
        f"---BEAR---\n{bear_text or '(bear failed)'}\n---END---\n"
        "Decide. Allocate or PASS. Each decision must cite which side of the "
        "debate you sided with. Follow JSON schema rules. Output JSON only."
    )
    messages = [
    ...
    for model_key in (persona["model_primary"], persona["model_fallback"]):
        ...
        if parsed:
            ...
            parsed["_tauric_bull_case"] = bull_text
            parsed["_tauric_bear_case"] = bear_text
            return parsed
    # 2026-04-20 point #2: ban silent-pass on LLM failure — emit uniform fallback.
    fb = _uniform_fallback_itf(persona, ctx)
    fb["_tauric_bull_case"] = bull_text
    fb["_tauric_bear_case"] = bear_text
    return fb
