...
- DAILY BET MANDATE: ≥3 allocations + ≥1 parlay each day.
  HARD REQUIREMENTS for any deviation:
    * If you emit ZERO allocations → "cash_rationale" MUST be ≥40 chars and cite
      a CONCRETE REASON (specific game data point, ref tendency, line move,
      injury, oracle conflict). "No edge today" / "passing" / "preserving" are
      INSUFFICIENT and will be flagged in audit. Empty rationale = forfeited day.
    * If you emit ZERO parlays → add field "parlay_refusal_reason" with ≥40 chars
      explaining why no parlay opportunity (e.g. "all top edges are correlated
      same-game, parlay would be variance not lift"). Without this field, server
      flags the day as audit-fail.
    * Every allocation MUST have rationale ≥30 chars citing concrete data.
      "Edge looks good" / "Model agrees" / "high-conviction" are REJECTED.
...
def run_experiment(progress=gr.Progress(track_tqdm=False)):
    ...
    for day_idx, day_date in enumerate(dates_sorted):
        ...
        def _agent_llm_worker(tid_cfg):
            ...
            # 2026-04-30 — TAURICRESEARCH BULL/BEAR/SYNTHESIS DEBATE CHAIN.
            # Each agent runs 3 sequential LLM calls (same persona, same data,
            # different system addendum). Inspired by TauricResearch/TradingAgents
            # (arXiv 2412.20138, 55.7K stars). The third pass parses as the final
            # decision; first two are persisted as text for full audit trail.
            bull_text = None
            bear_text = None
            raw = None

            # Bull/bear/synth share the SELFHOST_FAST endpoint by default to
            # avoid burning 3× rate limit on the agent's commercial provider.
            # If SELFHOST_FAST_PROVIDER env not set, fall back to agent's own
            # provider (3× rate-limit cost — fine for free-tier headroom).
            _bull_bear_provider = os.environ.get("TAURIC_DEBATE_PROVIDER", provider)

            # PASS 1 — BULL ANALYST
            try:
                bull_addendum = (
                    "\n\n=== TAURIC PASS 1/3: BULL ANALYST ===\n"
                    "You are now the BULL ANALYST. Forget your usual decision-making\n"
                    "role for this pass. Your only job: find the strongest bullish\n"
                    "case for tonight's slate. What's the high-conviction bet you'd\n"
                    "take if you HAD to bet? Cite specific data: player out, ref,\n"
                    "form streak, line move, oracle prediction, public fade.\n"
                    "Output 3-6 sentences of plain text. NO JSON, NO allocations.\n"
                    "Just the bullish thesis."
                )
                bull_text = _call_llm(_bull_bear_provider, system_prompt + bull_addendum, user_prompt,
                                     timeout=_nba_timeout, trace_name=f"nba-tauric-bull-{day_idx}",
                                     trace_metadata={"trader_id": tid, "day": day_date, "pass": "bull"})
            except Exception:
                bull_text = None
            ...
            try:
                bear_addendum = (
                    "\n\n=== TAURIC PASS 2/3: BEAR ANALYST ===\n"
                    "You are now the BEAR ANALYST. The BULL ANALYST has just argued:\n"
                    f"---BULL ARGUMENT---\n{bull_text or '(bull pass failed)'}\n---END BULL---\n"
                    "Your job: counter-argue every claim. What's wrong with the bull\n"
                    "case? What would make you fade these picks? Find the structural\n"
                    "weaknesses (correlation, IV crush, public-side trap, oracle\n"
                    "calibration error, late injury news, regression).\n"
                    "Output 3-6 sentences. NO JSON, NO allocations. Just the bear case."
                )
                bear_text = _call_llm(_bull_bear_provider, system_prompt + bear_addendum, user_prompt,
                                     timeout=_nba_timeout, trace_name=f"nba-tauric-bear-{day_idx}",
                                     trace_metadata={"trader_id": tid, "day": day_date, "pass": "bear"})
            except Exception:
                bear_text = None

            # PASS 3 — TRADER SYNTHESIS (this is the call that parses to JSON)
            synth_addendum = (
                "\n\n=== TAURIC PASS 3/3: TRADER (FINAL DECISION) ===\n"
                "You have read the BULL and BEAR analysts' arguments:\n"
                f"---BULL ARGUMENT---\n{bull_text or '(bull pass failed — proceed without it)'}\n---END BULL---\n"
                f"---BEAR ARGUMENT---\n{bear_text or '(bear pass failed — proceed without it)'}\n---END BEAR---\n"
                "Now decide as the TRADER. Allocate or PASS. Each allocation must\n"
                "have rationale citing which side of the debate you sided with and why.\n"
                "Follow the JSON schema rules above EXACTLY. Output JSON only."
            )
            try:
                raw = _call_llm(provider, system_prompt + synth_addendum, user_prompt,
                               timeout=_nba_timeout, trace_name=f"nba-tauric-synth-{day_idx}",
                               trace_metadata={"trader_id": tid, "day": day_date, "pass": "synth", "bankroll": ts["bankroll"]})
            except Exception:
                raw = None

            # Fallback provider if primary synth fails
            if not raw and cfg.get("fallback_provider"):
                try:
                    raw = _call_llm(cfg["fallback_provider"], system_prompt + synth_addendum, user_prompt,
                                   timeout=_nba_timeout, trace_name=f"nba-tauric-synth-fb-{day_idx}",
                                   trace_metadata={"trader_id": tid, "day": day_date, "fallback": True})
                except Exception:
                    pass
            return tid, raw, bull_text, bear_text

        _max_workers = min(len(TRADERS), int(os.environ.get("NBA_TF_LLM_POOL_WORKERS", "8")))
        _responses = {}
        _debate_texts = {}  # Tauric: tid → (bull_text, bear_text)
        ...
        try:
            # Tauric: timeout per-agent is 3× single timeout (3 sequential LLM calls)
            for _fut in as_completed(_futures, timeout=300.0):
                try:
                    _tid, _raw, _bull, _bear = _fut.result(timeout=1.0)
                    _responses[_tid] = _raw
                    _debate_texts[_tid] = (_bull, _bear)
                except Exception:
                    _responses[_futures[_fut]] = None
                    _debate_texts[_futures[_fut]] = (None, None)
        except Exception:
            pass
        ...
        for tid, cfg in TRADERS.items():
            ...
            # Tauric: pull bull + bear text from debate run
            _bull_text, _bear_text = _debate_texts.get(tid, (None, None))
            ts["llm_calls"] += 1
            if raw_response:
                ts["llm_ok"] += 1
            ...
            day_log = {
                ...
                # 2026-04-30 TAURIC: persist full debate text for audit transparency
                "tauric_bull_case": (_bull_text or "")[:4000],
                "tauric_bear_case": (_bear_text or "")[:4000],
                "tauric_passes_ok": int(bool(_bull_text)) + int(bool(_bear_text)) + int(bool(raw_response)),
            }
