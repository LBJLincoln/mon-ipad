...
print(f"[axelrod-ablation] NBA cfg={_AXELROD_ABLATION_CFG!r} A={AXELROD_MECH_A_ON} B={AXELROD_MECH_B_ON} C={AXELROD_MECH_C_ON}")
...
# Axelrod Mech B — one-sentence directive for each archetype (used in sacrificial suffix)
AXELROD_ARCHETYPE_DESCRIPTIONS: Dict[str, str] = {
    "pure_momentum": "Back teams on a 3+ game winning streak or ATS run; ride winners until they lose — no fading.",
    "mean_reversion": "Fade teams on 3-game hot streaks; back teams after 2-3 straight losses to regress toward true W%.",
    "news_event_driven": "Only bet games with a confirmed lineup change (injury/trade/suspension) announced within 24 h of tip.",
    "home_underdog_specialist": "Target home underdogs of +4 or more today only; skip all other game types.",
    "road_favorite_fade": "Fade road favourites of -3.5 or more; take the home team outright or on the spread.",
    "closing_line_value_only": "Only bet when your edge vs the closing line exceeds 4 %; if you can't beat the close, pass.",
    "injury_arbitrage": "Find games where a key player absence is under-priced by the market; bet the team that benefits.",
    "pace_inefficiency": "Back high-pace matchups where the posted total appears to undercount expected possessions.",
    "rest_differential": "Only take sides where one team has 2+ more rest days than the opponent.",
    "back_to_back_fade": "Fade every team on the second night of a back-to-back; back the rested opponent.",
    "revenge_narrative": "Back teams facing last season's playoff eliminator or a team that beat them by 15+ last meeting.",
    "divisional_hate": "Back division underdogs in intra-division matchups; familiarity closes the line inefficiency.",
    "national_tv_effect": "On ABC/ESPN/TNT slates, back the home underdog — national-stage pressure over-inflates chalk.",
    "pythagorean_divergence": "Bet teams whose Pythagorean W/L is 3+ games better than actual; fade teams in opposite direction.",
    "shot_chart_mismatch": "Back teams with high corner-3 rate against defences that surrender high corner-3 frequencies.",
    "ref_bias_per_team": "Identify crews with 2+ season home FT differential for a specific team; bet the biased direction.",
    "sharps_follow": "Only bet lines that have moved in your direction since open — line movement confirms sharp-money alignment.",
    "sharps_fade": "Fade public-consensus sides (60 %+ tickets) where the line drifts opposite to the public money.",
    "steam_chase": "Chase sudden aggressive line moves within 30 min of release — steam signals coordinated sharp action.",
    "reverse_line_movement": "Back teams where 60 %+ public fades them but the line moves toward them — books tracking sharp side.",
}
        ...
        for tid, _ts in ranked:
            ...
            if not allocs:
                ...
                for a in allocs:
                    ...
                    lines.append(
                        f"  {name}: game={a.get('game', '?')} pick={a.get('category', '?')} odds={a.get('odds', 0):.2f} "
        ...
        for _t2, _ in ranked:
            ...
            if _d2:
                ...
                for _a2 in _d2.get("allocations", []):
                    _pk = f"{_a2.get('game', '?')} {_a2.get('category', '?')}"
...
def build_sacrificial_system_suffix(archetype: str) -> str:
    ...
    description = AXELROD_ARCHETYPE_DESCRIPTIONS.get(
        archetype, "Apply this archetype consistently to every bet today."
    )
    return (
...
def compute_consensus_distance(tid: str, day_date: str, state: Dict, agent_logs: Dict) -> float:
    """Axelrod Mech C: KL divergence D_KL(agent || peers) over category bet distribution.
    ...
    for other_tid, logs in agent_logs.items():
        ...
        for a in day_log.get("allocations", []):
            ...
            if other_tid == tid:
                ...
                society[cat] += 1  # peers only: exclude self for true D_KL(agent||peers)
...
def write_axelrod_log(day_idx: int, day_date: str, state: Dict,
    ...
    try:
        ...
        rows = []
        # Axelrod Mech C: fleet-wide consensus pick distribution (how many agents picked each game)
        _cpd: Dict[str, int] = {}
        ...
        for tid, ts in state.items():
            ...
            rows.append({
                ...
                "consensus_pick_distribution": {g: _cpd[g] for g in {d.get("game", "") for d in decisions if d.get("game")}},
                "fleet": "nba",
