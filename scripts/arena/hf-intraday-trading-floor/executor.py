...
def submit(agent_tid: str, order: Dict[str, Any], last_quote: float) -> Dict[str, Any]:
    ...
    """
    # 2026-04-28 v8 — REMOVED v7 DTBP pre-flight refuse. Root cause was not
    # DTBP exhaustion; it was Alpaca account config `dtbp_check: 'entry'`
    # forcing every equity entry to compare cost_basis against DTBP=$400.
    # User flipped dtbp_check + pdt_check to 'exit' + ptp_no_exception_entry
    # to false → equity entries now use RegT BP ($36K) not DTBP. Old v7 refuse
    # would block entries the broker would now accept.
    positions = _load_positions()
