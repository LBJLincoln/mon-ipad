...
def pol_oracle_predict(event_id: str, event_features: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    event_features is the event dict matching political_engine.py v3.19
    (date, ticker, event_type, signal_strength, sector, agency, ...).
    ...
    # P7 /api/predict schema: {"events": [{event...}]}
    ev = dict(event_features or {})
    ev.setdefault("event_id", event_id)
    # If event_id has canonical date_ticker_type form, back-fill missing keys
    if "_" in event_id and not ev.get("date"):
        parts = event_id.split("_", 2)
        if len(parts) == 3:
            ev.setdefault("date", parts[0])
            ev.setdefault("ticker", parts[1])
            ev.setdefault("event_type", parts[2])
    payload = {"events": [ev]}
