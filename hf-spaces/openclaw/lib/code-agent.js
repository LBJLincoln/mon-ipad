...
class CodeAgent {
  constructor({ kimiApiKey, ghToken, ghOwner, vmBridge, bot, adminId, a2a }) {
    this.kimiApiKey = kimiApiKey || process.env.KIMI_API_KEY;
    ...
    this.a2a = a2a;
    this.agentName = process.env.AGENT_NAME || 'RGWA';
  ...
  // ══════════════════════════════════════════
  //  KIMI 2.5 LLM — OpenAI-compatible
  // ══════════════════════════════════════════

  async askKimi(messages, { maxTokens = 4096, temperature = 0.3 } = {}) {
    if (!this.kimiApiKey) throw new Error('KIMI_API_KEY not configured');
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Kimi API ${resp.status}: ${text.substring(0, 200)}`);
    }
  ...
  async executeTask(task) {
    if (!this.enabled) {
      return { success: false, error: 'Code agent not enabled (no KIMI_API_KEY)' };
    }
...
4. Potential risks`;
    ...
    return this.askKimi([{ role: 'user', content: prompt }], { maxTokens: 1000 });
  }
...
Generate clean, production-ready code. Include only files that need changes.`;
    ...
    const result = await this.askKimi([{ role: 'user', content: prompt }], {
      maxTokens: 4096,
      temperature: 0.2,
    });
