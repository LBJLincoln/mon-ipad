 ...
 * 2. Fall back to DNS-over-HTTPS (Cloudflare) for any unresolvable domain
 *
 * Adapted from democra-ai/HuggingClaw.
 *
 * Loaded via: NODE_OPTIONS="--require /app/scripts/dns-fix.cjs"
 */
"use strict";
...
// Pre-resolved domains (populated by dns-resolve.py at startup)
let preResolved = {};
try {
  ...
  // File not ready yet — proceed without pre-resolved cache
}
...
dns.lookup = function patchedLookup(hostname, options, callback) {
  ...
  // Skip for localhost, IPs, internal domains
  if (
    !hostname ||
    hostname === "localhost" ||
    hostname === "0.0.0.0" ||
...
console.log("[dns-fix] DNS lookup patched with DoH fallback");
