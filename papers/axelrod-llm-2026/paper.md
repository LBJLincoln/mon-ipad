...
giving the game a *truth-inducing* character [@gneiting2007strictly].
In practice, LLM outputs are generated by autoregressive sampling rather than
explicit expected-score optimisation; truth-induction is therefore a normative
design goal and a property of the incentive structure, not a mechanical guarantee
over LLM behaviour.
...
*and* agent $i$ has not undergone SRR within the most recent $W_{\text{persist}}$ days
(*cooldown constraint*). The consecutive-day requirement prevents transient losses from
triggering unnecessary reallocations; the cooldown prevents re-evaluation before the
reallocation persistence window has elapsed, ensuring each reallocation has a fair
assessment period. We set $\delta_{\text{sac}} = 0.02$ and $W = 7$ based on
...
> [@aumann1959acceptable] in the societal Brier minimisation game restricted to
> the set of sacrifice-eligible agents: no coalition
> $\mathcal{C} \subseteq \mathcal{I}_{\text{elig},d}$, where
> $\mathcal{I}_{\text{elig},d} = \{i \in \mathcal{I} : i \text{ is sacrifice-eligible at day } d\}$,
> can jointly refuse SRR and (weakly) improve the ensemble Brier of $\mathcal{C}$
> while (weakly) reducing individual Brier for all members of $\mathcal{C}$.
> (Non-eligible agents have no SRR action available and therefore cannot participate
> in a meaningful coalitional deviation; the relevant strategy space is restricted to
> $\mathcal{I}_{\text{elig},d}$.)
...
> *Figure 1: Day-Bucket v3 pipeline. Each trading day proceeds through five
> sequential stages: (1) morning council brief distributed to all $N$ agents;
> (2) sealed 15-minute prediction window; (3) event resolution and per-agent
> Brier scoring; (4) end-of-day common-knowledge broadcast ($\Omega_d$ only;
> peer predictions remain sealed until the next morning council); (5) SRR
> eligibility evaluation and archetype reallocation for qualifying agents.
> The SRR stage (shaded) is absent in Conditions B (Fixed Ensemble), C (DMAD-Static),
> and E (Free-Rider selects a random non-eligible agent); partially active in
> Condition D (Sham-SRR: label updated, system prompt unchanged). Implementation:
> `scripts/arena/hf-llm-trading-floor/app.py`.*
> **[FIGURE PENDING: `scripts/plots/day_bucket_architecture.py`]**
...
The moderator role rotates on a weekly round-robin basis to prevent
...
(the cooldown constraint formalised in the eligibility definition of §3.4).
The archetype update is applied by atomically replacing the agent's runtime
system-prompt configuration without interrupting the inference service
